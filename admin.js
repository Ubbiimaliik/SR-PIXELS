...
async function fetchFolders() {
  try {
    const timestamp = new Date().getTime();
    const res = await fetch(`/api/folders?t=${timestamp}`);
...
async function fetchImages() {
  ...
  try {
    const timestamp = new Date().getTime();
    const res = await fetch(`/api/portfolio?folderId=${currentFolderId}&t=${timestamp}`);
    ...
    images.forEach(img => {
      grid.innerHTML += `
        <div class="portfolio-item">
          <img src="/api/portfolio/${img._id}/image?t=${new Date().getTime()}" alt="${img.name}" />
          <p>${img.name}</p>
